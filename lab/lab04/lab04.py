...
def distance(city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    lat_city1,lat_city2 = get_lat(city1), get_lat(city2)
    lon_city1,lon_city2 = get_lon(city1), get_lon(city2)
    return sqrt((lat_city2 - lat_city1) ** 2 + (lon_city2 - lon_city1) ** 2)
...
def put_piece(board, max_rows, column, player):
    ...
    "*** YOUR CODE HERE ***"
    i = max_rows - 1
    while get_piece(board,i,column) != '-':
    	if i == 0:
    		return -1,board
    	i = i - 1
    board_lst = replace_elem(board[i],column,player)
    new_board = replace_elem(board,i,board_lst)
    return i,new_board
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
    	...
    	for col in range(max_cols):
    		current_value = get_piece(board,row,col)
    		current_row = current_row + current_value + ' '
